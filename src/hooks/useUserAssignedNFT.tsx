...
const useAssignesNftHooks = (): {
    ...
    const callingTheNestedData = (index: number) => {
        ...
        resNFTs.get_all_nfts(
            index, true, userToken.token).then(data => {
    ...
    const prefixedPaginate = (it: number) => {
        ...
        respFaqs.get_all_nfts
            (it, true, userToken.token).then(data => {
                setLoadingData(false)
                setnftsData(data)
            })
    ...
    const initial_fetching_nfts = () => {
        setLoadingData(true)
        let resNFTs = new NftsAPI()
        resNFTs.get_all_nfts(1, true, userToken.token).then(data => {
            setnftsData(data)
            setLoadingData(false)
        })
    }
...
export default useAssignesNftHooks